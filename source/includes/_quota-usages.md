...
```shell
curl {{ api_base_url }}/v2/quota-usages/flows/{FLOW_ID}/history?from=2019-09-01&to=2019-10-01 \
   -u {EMAIL}:{APIKEY}
```
...
`GET {{ api_base_url }}/v2/quota-usages/flows/{FLOW_ID}/history?from={DATE_FROM}&to={DATE_TO}&group_by={GROUP_BY}`
